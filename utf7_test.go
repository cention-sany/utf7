...
var encode = []struct {
	...
	{"\x20\x25+\x27\x7E", ` %+-'+AH4-`, true},
	{"\x1F\x20+\x7E\x7F", "+AB8 +-+AH4Afw-", true},
	...
	// RFC 3501
	{"~peter/mail/\u53F0\u5317/\u65E5\u672C\u8A9E", "+AH4-peter/mail/+U/BTFw-/+ZeVnLIqe-", true},
	{"~peter/mail/\u53F0\u5317/\u65E5\u672C\u8A9E", "+AH4-peter/mail/+U/BTFw-/+ZeVnLIqe-", true},
	{"\u263A!", "+Jjo!", true},
...
var irreversible_decode = []struct {
	in  string
	out string
	ok  bool
}{
	{"1 +- 1 +AD0 2", "1 + 1 = 2", true},
	// ASCII in Base64
	{"+AGE-", "a", true},
	{"+ACY-", "&", true},
	{"+AGgAZQBsAGwAbw-", "hello", true},
	{"+JjoAIQ-", "\u263A!", true},
	{"+Jjo", "\u263A", true},
...
func TestUTF7Decode(t *testing.T) {
	...
	for _, test := range irreversible_decode {
		out, err := UTF7Decode(test.in)
		if out != test.out {
			t.Errorf("UTF7Decode(%+q) expected %+q; got %+q", test.in, test.out, out)
		}
